...
summary_dtm_k20 <- readRDS(paste0(data_path, "summary_dtm_k20"))

summary_dtm_k20_tidied <- tidy(summary_dtm_k20)

#write_rds(sotu_lda_k16, "lda_16.rds")
summary_dtm_k20_tidied %>% glimpse()

top_terms_k20 <- summary_dtm_k20_tidied %>%
   group_by(topic) %>%
   slice_max(beta, n = 10, with_ties = FALSE) %>%
   ungroup() %>%
   arrange(topic, -beta)

tp_k20 <- top_terms_k20 %>%
...
ggsave("tp_k20.png", device = "png", plot = tp_k20, path = data_path)
